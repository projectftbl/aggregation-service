...
Scheduler.prototype.schedule = function(accountId, options) {
  ...
  return getAccount(accountId, this.context).then(function(account) {
    if (account.schedule == null) return;

    if (account == null) return next.call(that, { id: accountId, schedule: DELAY }, 'schedule');
    if (opts.force) return next.call(that, account, 'schedule');
